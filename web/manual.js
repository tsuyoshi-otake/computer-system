...
ls -la /</code></pre></section>
  ...
  },
  {
    ...
    html: `
      ...
      <section class="manual-section"><h3>Production execution rate</h3><p>The Bedrock host admits approximately one half of each model's persisted nominal clock: 400,000 cycles per 20 Hz tick for CS386SX 16 MHz, 825,000 for CS486DX 33 MHz, and 1,650,000 for CS486DX2 66 MHz. The build-validated Behavior Pack setting <code>guestRealtimeDivisor</code> controls this ratio; its shipped value is 2, while 1 requests realtime admission. Rebuild the packs and restart BDS after changing it. Managed BDS assigns CS386SX and CS486 Computers to the same fixed worker pool. Each worker and the local Bedrock lane admit at most 1,650,000 modeled cycles and 1,650,000 machine instructions per tick, so one resource can sustain at most 33 MHz of aggregate modeled CPU work before fair sharing. Each resource owns an independent round-robin cursor, so uneven worker placement does not bias long-term sharing among the Computers assigned to that worker. Worker count changes aggregate host concurrency but never changes a model, clock, cycle debt, or per-Computer guest admission rate. Host contention may reduce the observed rate but never increases guest timing above this admission ratio.</p></section>
      ...
      <section class="manual-section manual-grid-2"><div><h3>2.4 Memory model</h3><p>RAM is a checked, little-endian linear byte array. <code>LOAD</code> and <code>STORE</code> transfer signed 32-bit words. CS386SX exposes a 24-bit address bus and a 16-bit data bus, with 2 MiB provided by two 1 MiB 30-pin SIMM DRAM modules. An even-addressed dword needs two 16-bit transfers; an odd-addressed dword needs three and records an unaligned access. CS386SX has no cache, so every one of those transfers pays the full main-memory cycle cost; CS486DX/DX2 pay that same per-transfer cost only when their L1/L2 misses, so a cache hit stays inexpensive. CS486DX uses four 512 KiB 30-pin SIMMs, while CS486DX2 uses two 4 MiB 72-pin SIMMs. The stack begins at the top of RAM and grows downward.</p><p>Memory reports include bounded OS overhead as well as live guest data. CS-Linux separates resident kernel, system services, buffers, and guest runtime. CS-DOS separates DOS system/drivers and guest runtime while preserving conventional, upper, reserved, and XMS region totals. Version 3 executables declare their <code>cs-flat32-v1</code> stack, heap, and auxiliary residency before process construction. The built-in empty-startup shell uses one 64 KiB composite grant; user-authored startup uses one quarter of physical RAM capped at 1 MiB, and foreground Python keeps its 1 MiB managed-runtime quota. Older version 1/2 executables remain readable but take the complete currently free physical remainder exclusively.</p></div><div class="manual-memory-map"><span>LOW ADDRESS</span><b>OS resident memory</b><b>compiler variables / heap</b><b>free space</b><b>↓ stack growth</b><span>HIGH ADDRESS</span></div></section>
      <section class="manual-section"><h3>2.5 Model-specific cycle classes</h3><table><thead><tr><th>Class</th><th>CS486DX / CS486DX2</th><th>CS386SX</th></tr></thead><tbody><tr><td>Simple ALU</td><td>1</td><td>2</td></tr><tr><td>LOAD / STORE base</td><td>2 / 2 plus cache and write-through</td><td>6 / 4 plus main-memory cycles every access, no cache</td></tr><tr><td>PUSH / POP base</td><td>2 / 2 plus cache and write-through</td><td>4 / 6 plus main-memory cycles every access, no cache</td></tr><tr><td>Conditional branch</td><td>3 taken / 1 not taken</td><td>7 taken / 3 not taken</td></tr><tr><td>CALL / RET</td><td>3 / 3</td><td>9 / 12</td></tr><tr><td>MUL</td><td>9</td><td>9–38, operand early-out</td></tr><tr><td>DIV / MOD</td><td>40</td><td>43</td></tr><tr><td>PRINT</td><td>8 + payload</td><td>12 + 16-bit bus payload</td></tr></tbody></table><p>At 20 ticks per second the nominal clocks correspond to 1,650,000 cycles/tick for the standard desktop, 3,300,000 for Advanced Desktop, and 800,000 for portable. Production divides those admission budgets by the configured <code>guestRealtimeDivisor</code>. CS486DX and CS486DX2 have a cold 8 KiB four-way unified L1 with 16-byte lines and write-through stores; Advanced adds a 256 KiB external L2. CS386SX has neither cache, so it pays the same per-transfer main-memory cost as CS486DX/CS486DX2 on every access instead of only on a cache miss. Cache tags, recency, prefetch state, and counters are transient per process. Every access remains O(1).</p></section>
...
sudo cat /var/log/auth.log</code></pre><p><code>/proc/&lt;pid&gt;/cmdline</code>, <code>stat</code>, and <code>status</code>, plus <code>/proc/self/*</code>, <code>loadavg</code>, and mount/device/service views all come from one per-Computer OS state. The journal defaults to 256 records, 32 KiB total, and 1 KiB per record. It is a rotating log, not a fixed table: reaching a limit drops the oldest records instead of failing the append, so a full journal never blocks a login, a shutdown, or a boot. Every view that renders it — <code>dmesg</code>, <code>/var/log/messages</code>, and <code>/var/log/auth.log</code> — then begins with one <code>-- N earlier record(s) dropped by journal rotation --</code> line, so the loss is stated rather than silent. Retained records keep their original order and the count survives cold restore; restoring a snapshot whose journal is larger than the current limits truncates the oldest records the same way. The device registry includes tty/console/null/zero, the fixed disk, and the removable state-backed floppy; none are host device files.</p></section>
      <section class="manual-section"><h3>4.11 Sync, graceful stop, and safe recovery</h3><p><code>sync</code> crosses the real Computer persistence boundary. Shutdown and reboot stop new admission, signal owned work, drain already-admitted block I/O, save data, unmount, stop services/devices, then record <code>final sync requested</code> and that the intent is prepared for final persistence before saving the final cold projection. Only then may the machine power off or reboot. The two neutral terminal records survive cold restore exactly once; no unsaved success line is added after the callback. If marker creation or the callback fails, only that attempt's provisional markers are removed before the fault is published. A later automatic dirty save can retain the fault but cannot turn the failed final boundary into apparent success. Each phase has a 200-tick deadline, and a durability or drain failure faults visibly instead of claiming a clean stop.</p><p>A boot that fails, and a shutdown that fails, both leave the reason on screen. A failed power-on or safe boot replaces the screen with a CSBIOS halt screen: the POST rows the machine actually reached, a <code>Boot Error</code> row naming the failing phase, the reason wrapped into at most two rows, and a final row naming the recovery that machine really has. A failed shutdown instead appends <code>Halt failed during &lt;phase&gt;.</code>, the reason, and the same halt line under the guest's own shutdown output, so nothing already on screen is destroyed. A halted machine accepts no input, so it shows no cursor. The block face stays dark in both cases because a fault releases VRAM; open the Computer, or attach the Web Terminal, to read the screen.</p><p>The runtime has a one-shot safe-boot boundary. It is available only while the Computer is <code>crashed</code>. In Minecraft, open the crashed Computer normally to see the recovery instruction, then sneak while opening it to safe boot. On an attached Web Terminal, the crashed-state power control changes to safe boot. Safe boot is unavailable from the guest shell and MCP command path.</p><p>What safe boot changes depends on the machine. On a Desktop or Advanced Desktop running CS-Linux it preserves but bypasses a broken <code>/startup.py</code> and records that choice in the boot journal; the exact startup file is never reset, renamed, or edited. A Portable CS386SX has no user MicroPython, so it has no <code>/startup.py</code> to bypass: there, safe boot only retries without bootable floppy media, and the halt screen, the in-game recovery line, and the boot journal all say so instead of claiming a bypass.</p></section>
...
perl -lane 'print "$F[0]:$F[2]"' /etc/passwd
tar -cf project.tar project
tar -tf project.tar
gzip -k project.tar
zip project.zip project
unzip -l project.zip</code></pre><p><code>sed</code> supports <code>-n</code>, <code>-e</code>, numeric, last-line, and pattern addresses, plus bounded <code>p</code>, <code>d</code>, and <code>s</code> commands. <code>awk</code> supports <code>-F</code>, <code>BEGIN</code>/<code>END</code>, pattern and comparison rules, <code>print</code>/<code>printf</code>, <code>$0</code>-<code>$64</code>, <code>NR</code>, and <code>NF</code>. Both use the guest's own bounded pattern parser—literals, dot, star, anchors, and character classes—not host JavaScript regular expressions. Programs are limited to 4,096 bytes and 32 rules; input is limited to 256,000 bytes and 4,096 records.</p><p>CS-Linux rootfs v20 installs <code>/usr/bin/perl</code>; CS-DOS does not. It is a bounded interpreter for a Perl 5.40 surface, not a port of upstream perl and not a CPAN or POSIX conformance claim. It supports <code>use strict</code>, <code>use warnings</code>, <code>use v5.40</code>, <code>say</code>, scalars, arrays, hashes, <code>my</code> scoping, subroutines and recursion, statement modifiers, <code>foreach</code> aliasing, <code>sort</code>/<code>grep</code>/<code>map</code> blocks, <code>sprintf</code>/<code>printf</code>, <code>split</code>, <code>join</code>, <code>tr///</code>, <code>m//</code>, <code>s///</code>, <code>eval BLOCK</code> with <code>$@</code>, <code>die</code>/<code>warn</code>/<code>exit</code>, here-documents including <code>&lt;&lt;~</code>, loop labels with <code>next</code>/<code>last</code>, <code>s///e</code> and <code>s///r</code>, <code>tr///cdrs</code>, real <code>undef</code> with <code>//</code>, and the <code>-e -n -p -l -a -F -c -w -v</code> switches. Three-argument <code>open</code> reads and writes guest files only, through the same credentialed filesystem as every other command.</p><p>Matching uses a guest-owned backtracking engine with alternation, groups and captures, greedy and non-greedy <code>* + ? {n,m}</code>, classes, escapes, anchors, <code>\\b</code>, and the <code>g</code>/<code>i</code>/<code>m</code>/<code>s</code> flags under a fixed step budget—never a host regular expression. Program text is limited to 64 KiB and 4,096 statements; a run is limited to 1,000,000 steps, 1 MiB of input, 1 MiB of output, 8 open handles, and a call depth of 64, and exceeding one fails explicitly with exit status 2. Hash iteration is insertion-ordered and there is no wall clock, so a program's output is a deterministic function of its input. Modules outside the pragma allowlist, references and nested data structures, <code>bless</code> and <code>-&gt;</code>, <code>eval STRING</code>, <code>local</code>, prototypes, <code>goto</code>, <code>tie</code>, <code>format</code>, backticks, <code>fork</code>, <code>exec</code>, and <code>system</code> are rejected at compile time instead of approximated.</p><p>The filesystem stores and accounts arbitrary bytes, so archive members survive snapshot and copy without UTF-8 conversion. <code>tar</code> creates, lists, and atomically extracts bounded ustar archives. <code>gzip</code>/<code>gunzip</code> use deterministic RFC 1952 stored-DEFLATE streams; other DEFLATE block methods fail explicitly. <code>zip</code>/<code>unzip</code> support standard UTF-8, unencrypted method-0 archives; encryption, ZIP64, and compressed methods fail explicitly. One archive is at most 1 MiB, with 512 entries, 32 operands, path depth 32, and 4 MiB expanded data. Extraction rejects absolute paths, traversal, duplicates, symlink pivots, conflicts, and malformed checksums before committing any entry.</p></section>
      <aside class="manual-callout"><b>Bare Perl source input</b><p><code>perl</code> with no program reads program source, not runtime data, from standard input. A pipe or <code>&lt;</code> redirection executes once EOF is reached. At the controlling terminal, enter source lines and press <kbd>Ctrl</kbd>+<kbd>D</kbd> to compile and run them; <kbd>Ctrl</kbd>+<kbd>C</kbd> cancels without execution. Collected source is excluded from shell and browser history, is capped at 64 KiB and 4,096 lines, and is never replayed as the program's runtime standard input.</p></aside>
...
probe &gt;all 2&gt;&amp;1
probe 2&gt;&amp;1 &gt;out
probe 2&gt;err |&amp; more</code></pre><p>Every foreground stage is a scheduler-owned process in one process group. Adjacent stages exchange <code>Uint8Array</code> data through a fixed 4 KiB ring: empty readers and full writers yield, every writer reference must close before EOF, and writing after the final reader closes produces SIGPIPE status 141. The pipeline status is the last stage status; <code>pipefail</code> and background pipelines are unavailable. Redirect setup is validated and committed before a stage starts, <code>&gt;</code> truncates at shell-open time, repeated operations are bounded to 16 per stage, and later runtime disk failure may leave only the bytes already committed.</p><p><code>less</code> and <code>more</code> are supported only as the final terminal-owned stage. They accept one file, <code>&lt;</code>/pipe stdin, or <code>-</code>; navigation keys remain on the controlling-terminal channel and never become document bytes. Live input reports an unfinished length honestly and retains at most 64 KiB. <code>q</code> or Escape closes the reader, lets an active producer observe SIGPIPE, and resumes a saved <code>&amp;&amp;</code>/<code>||</code>/<code>;</code> continuation exactly once. Parent-mutating commands and other terminal-exclusive programs are single-stage-only.</p></section>
...
cat /proc/1/cmdline</code></pre><p>These entries are read-only views generated from the Computer identity, hardware, clock, process, service, device, and mount model. Every live PID exposes bounded <code>cmdline</code>, <code>stat</code>, and <code>status</code> views. They are not host Linux files. Directory and hard-link indexes keep ordinary listings O(N); no command scans the host filesystem.</p></section>
  ...
  },
  {
    ...
    html: `
      ...
      <aside class="manual-callout"><b>Shell completion</b><p>At a CS-Linux or CS-DOS shell prompt, Tab completes commands, directories, files, and virtual devices without running the line. A unique match or shared prefix is inserted immediately. If ambiguity remains, the guest shell writes at most 64 matches in bounded columns inside its fixed 80x25 terminal, adds <code>...</code> when more matches exist, and redraws the prompt with the same unsubmitted draft. Completion therefore behaves like an OS terminal operation: candidates are not exposed as a browser shelf or selectable DOM state.</p><p>CS-Linux path matching is case-sensitive and uses slashes. CS-DOS matching is case-insensitive and displays uppercase drive paths with backslashes. Secret input never completes. Input is briefly locked while the current writer's request is pending; editing, writer loss, session changes, timeouts, and late companion responses cannot apply stale text.</p></aside>
...
PATH C:\\DOS
SET TEMP=C:\\TEMP
SET INCLUDE=C:\\INCLUDE</code></pre><table><thead><tr><th>Command</th><th>Contract</th></tr></thead><tbody><tr><td>SET [NAME[=VALUE]]</td><td>List, query, assign, or remove a case-insensitive environment variable</td></tr><tr><td>PATH [directories]</td><td>Show or replace up to the first 16 semicolon-separated search entries</td></tr><tr><td>PROMPT [template]</td><td>Set at most 64 characters; <code>$P $N $G $L $B $Q $V $_ $$</code> are recognized</td></tr><tr><td>ECHO ON / OFF</td><td>Control batch command echo; a leading <code>@</code> suppresses one line</td></tr><tr><td>REM text</td><td>Comment with no output or side effect</td></tr></tbody></table><p>DOS pipelines are sequential: each edge is a uniquely owned strict-8.3 spool under <code>C:\\TEMP</code>. A producer completes into the guest filesystem before the next stage starts; consumed spools are deleted as the pipeline advances, and setup, quota, interruption, disconnect, or reboot cleanup never uses a host temporary file. <code>C:\\DOS\\MORE.COM</code> accepts one file, redirected stdin, or pipe input as the final interactive stage. <code>DIR | MORE</code>, <code>TYPE file | MORE</code>, <code>MORE file</code>, and <code>MORE &lt; file</code> are supported; <code>LESS</code> and Linux-only fd syntax are not.</p></section>
...
RUN --STATS SCATTER</code></pre><p>The second statistics line reports L1 and L2 hits/misses, bus transfers, unaligned dwords, and pipeline flushes. Every run starts cold, so comparisons are reproducible and do not inherit another program's cache.</p></section>
      ...
      <section class="manual-section"><h3>13.6 Batch programs with <code>run --batch</code></h3><p><code>run [--batch] [--stats] program [arguments ...]</code> is CS-Linux only. <code>--batch</code> is a declaration you make about the program: <b>this process uses no operating-system service</b>. A batch process may still receive <code>argv</code>, the environment, and the working directory through the ordinary CS ABI startup image, ask for its heap placement, write to standard output and standard error, and exit with a status. Every other CS ABI operation - opening or reading a file, terminal size and presence, key waits and polls, the clock, and sleeping - fails immediately with <code>UnsupportedOperationError</code> naming the operation, and the message tells you to re-run the program without <code>--batch</code>. Nothing is approximated and no service is silently substituted.</p><p>Declaring a program batch lets the host run it on the managed compute plane instead of inside the interactive scheduler lane, so a computation-heavy program finishes in less wall-clock time. It does not change one guest-visible number: instructions, modeled cycles, cache and bus counters, registers, output bytes, and exit status are identical with and without the flag. Compare <code>run --stats bench</code> and <code>run --batch --stats bench</code> and expect the same statistics line. Where a batch program actually runs remains a server operator's configuration, not a guest property.</p><p>Batch output is one ordered stream: fd 1 and fd 2 interleave in exact write order, fd 1 is not line buffered, and the whole stream is delivered when the program finishes. Because of that, <code>--batch</code> is refused before anything runs when it is combined with a pipeline or a redirect, when the command is put in the background with <code>&amp;</code>, when the target is a shell utility rather than a compiled program, and when the executable is not a hosted CS-Linux program with a <code>main</code> symbol. CS-DOS <code>run</code> has no batch concept and rejects the option as a usage error.</p><pre><code>cc bench.c -o bench
run --stats bench
run --batch --stats bench   # same output, same cycles, less waiting
run --batch reader          # fails: batch process cannot use CS ABI operation 8</code></pre><aside class="manual-warning"><b>Interactive programs gain nothing</b><p>An interactive program is limited by how fast you type, not by how fast it computes. <code>nethack</code>, <code>vi</code>, and a Python REPL are input bound and could not run as batch programs anyway, because they need the terminal and the keyboard the batch contract refuses.</p></aside></section>
...
echo $?             # previous status</code></pre></section>
      <section class="manual-section"><h3>16.6 Canonical numeric ceilings</h3><table><thead><tr><th>Resource</th><th>Ceiling</th><th>Observable outcome</th></tr></thead><tbody><tr><td>Direct CS486 run</td><td>10,000 instructions per bounded direct run</td><td>Status 124 / yielded</td></tr><tr><td>CS-DOS batch</td><td>256 lines; nesting depth 8</td><td>Explicit line-limit or depth status</td></tr><tr><td>CONFIG.SYS</td><td>64 lines</td><td>Explicit unsupported or invalid directive</td></tr><tr><td>CS-Linux pipe / redirects</td><td>4 KiB per edge / 16 redirects per stage</td><td>Backpressure or explicit syntax/setup failure</td></tr><tr><td>Linux live pager history</td><td>64 KiB</td><td>Explicit pager input limit</td></tr><tr><td>DOS pipeline spool/output</td><td>Guest disk quota / 256,000 characters</td><td>Cleanup plus nonzero ERRORLEVEL</td></tr><tr><td>DOS TREE</td><td>512 entries; 32 levels</td><td>Explicit traversal limit</td></tr><tr><td>Manual search</td><td>24 section results</td><td>Highest-ranked bounded results</td></tr></tbody></table></section>
...
const terminalLineHandoffAnchor =
...
function authoredChapterHtml(id, html) {
  ...
  return html.replace(
    terminalLineHandoffAnchor,
    `${terminalLineHandoffCallout}${terminalLineHandoffAnchor}`,
  );
}
...
export const manualChapters = publicationOrder.map((id, index) => {
  ...
  const html = enrichHeadings(
    authoredChapterHtml(id, rawChapter.html),
    id,
    number,
  );
  return {
