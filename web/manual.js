...
sudo cat /var/log/auth.log</code></pre><p><code>/proc/&lt;pid&gt;/cmdline</code>, <code>stat</code>, and <code>status</code>, plus <code>/proc/self/*</code>, <code>loadavg</code>, and mount/device/service views all come from one per-Computer OS state. The journal defaults to 256 records and 32 KiB total. The device registry includes tty/console/null/zero, the fixed disk, and the removable state-backed floppy; none are host device files.</p></section>
      ...
      <section class="manual-section"><h3>4.11 CS Make 1.0</h3><p>Current CS-Linux rootfs v8 installs <code>/usr/bin/make</code>. CS-DOS does not install MAKE; its separate bounded project workflow remains CS PROGRAM LIST and PWB. Run <code>man make</code> for the exact command synopsis and limits.</p><pre><code>CC = cc
app: main.o
\tld main.o -o app
main.o: main.c
\t$(CC) -c $&lt; -o $@
...
.PHONY: clean
clean:
\trm -f app main.o</code></pre><p>CS Make accepts an explicit Makefile or makefile, a project directory, dry-run, force-rebuild, and silent options, command-line variable overrides, explicit dependency rules, the four common assignment forms, <code>.PHONY</code>, parenthesized or braced variable references, doubled dollar signs, and automatic target/first-prerequisite/all-prerequisites variables. The first non-special rule is the default. Guest mtimes and bounded <code>CSMAKE2</code> records in <code>.cs-make-state</code> skip a target only while its SHA-256 input, output, recipe, and toolchain identities still match. Missing, evicted, legacy, or foreign-toolchain records rebuild safely; malformed state fails explicitly. Each completed target records state after recipe I/O and post-build verification, so a later target failure retains earlier successful targets.</p><p>Planning is bounded to 32,768 source characters, 256 lines, 128 rules, 256 graph nodes, 512 edges, 64 prerequisites per rule, graph depth 32, 256 recipe lines, and 1 MiB of fingerprint input. Parsing, planning, and fingerprint reads begin after one make PID and one 128 KiB guest RAM lease are admitted. The initial scheduler step performs planning, then at most one isolated recipe advances per later tick. Only <code>as</code>, <code>cc</code>, <code>c++</code>, <code>ld</code>, <code>nm</code>, <code>objdump</code>, <code>cp</code>, <code>mv</code>, <code>rm</code>, <code>mkdir</code>, <code>rmdir</code>, <code>touch</code>, <code>echo</code>, and <code>printf</code> are admitted. Pipelines, redirects, chains, background or TUI/session/lifecycle work, recursive make, implicit rules, includes, conditionals, parallel jobs, and every host command fail explicitly. Ctrl+C, final disconnect, failure, and shutdown each have one finalization owner that reaps the PID and returns the lease exactly once.</p></section>
