...
echo $?</code></pre></section>
      ...
      <section class="manual-section"><h3>15.5 Storage migration after an update</h3><p>An older world may pause Computer interactions while BDS converts indexed Dynamic Property pages and inline filesystem files. Progress is reported as <code>CS_STORAGE_MIGRATION</code>. Each Computer is written and verified before the identity registry is switched, so an interruption keeps the older complete generation readable and the next start safely resumes by rescanning and skipping completed Computers.</p><ol class="manual-procedure"><li>Stop BDS before copying or restoring a world; never copy a live LevelDB directory.</li><li>Back up the complete world directory before the first start with a storage-format update.</li><li>Start BDS and wait for the migration-complete message before using Computers or running headless probes.</li><li>If migration fails, stop BDS, preserve the logs and failed world, then restore the stopped backup or correct the reported corrupt property.</li></ol><p><b>Verify:</b> Find a terminal <code>CS_STORAGE_MIGRATION</code> record with <code>state</code> equal to <code>complete</code>, then open the same Computer and inspect its files.</p><p><b>Expect:</b> The Computer ID, OS profile, hardware, terminal state, and guest files are unchanged.</p></section>
