...
halt</code></pre></section>
  ...
  },
  {
    id: "micropython",
    html: `
      <header class="manual-page-header"><p class="manual-kicker">Chapter 06 · Desktop language runtime</p><h2>Computer System Python on CS486</h2><p class="manual-lead">On the desktop CS486DX and CS486DX2 profiles, the MicroPython-compatible language compiles to the same resumable guest process as C, C++, and ASM; there is no separate Python VM. Source in <code>/startup.py</code> starts when the Computer powers on. The portable CS386SX DOS profile does not expose user MicroPython.</p></header>
...
sudo cat /var/log/auth.log</code></pre><p><code>/proc/&lt;pid&gt;/cmdline</code>, <code>stat</code>, and <code>status</code>, plus <code>/proc/self/*</code>, <code>loadavg</code>, and mount/device/service views all come from one per-Computer OS state. The journal defaults to 256 records and 32 KiB total. The device registry includes tty/console/null/zero, the fixed disk, and the removable state-backed floppy; none are host device files.</p></section>
  ...
  },
  {
    id: "basic",
    html: `
      <header class="manual-page-header"><p class="manual-kicker">Chapter 11 · CS QBASIC</p><h2>DOS editor and bounded QBasic source</h2><p class="manual-lead">CS-DOS alone ships the original, sandboxed <code>QBASIC.EXE</code>. It targets Microsoft QBasic 1.1 command and source behavior where listed here, but it is not Microsoft's binary, help database, artwork, 16-bit x86 runtime, or a native DOS interrupt environment. Current CS-Linux exposes neither <code>basic</code> nor <code>basicc</code>.</p></header>
      <section class="manual-section"><h3>11.1 Starting CS QBASIC</h3><pre><code>QBASIC                         REM open the Welcome dialog
...
QBASIC /EDITOR C:\\README.TXT  REM the same editor engine used by EDIT</code></pre><p>The accepted switches are <code>/B</code>, <code>/EDITOR</code>, <code>/G</code>, <code>/H</code>, <code>/MBF</code>, <code>/NOHI</code>, and <code>/RUN</code>. Unknown and duplicate switches, more than one filename, <code>/RUN</code> without a file, and <code>/EDITOR</code> combined with <code>/RUN</code> fail visibly. <code>/HELP</code> is not invented.</p></section>
      <section class="manual-section"><h3>11.2 Shared IDE, EDIT, keyboard, and mouse</h3><p><code>EDIT file</code> and <code>QBASIC /EDITOR file</code> use one bounded session implementation for menus, buffer, save/error handling, search, undo, dirty-exit confirmation, pointer hit testing, drag selection, and <code>Ctrl+C</code>/<code>Ctrl+X</code>/<code>Ctrl+V</code>. Web Terminal mouse events are accepted only from the current writer; cell coordinates and sequences are checked, moves are coalesced, and pressed buttons are released on takeover, range loss, and disconnect.</p><table><thead><tr><th>Key</th><th>Current result</th></tr></thead><tbody><tr><td>Shift+F5</td><td>Compile and run from the start</td></tr><tr><td>F4</td><td>Show or hide bounded program output</td></tr><tr><td>F9</td><td>Set or clear a visible breakpoint marker; source debugging is not yet connected</td></tr><tr><td>F5 / F7 / F8 / F10</td><td>Report that Continue, Run to Cursor, Step, or Step Over is not implemented; they never silently rerun the program</td></tr></tbody></table></section>
...
cat /proc/1/cmdline</code></pre><p>These entries are read-only views generated from the Computer identity, hardware, clock, process, service, device, and mount model. Every live PID exposes bounded <code>cmdline</code>, <code>stat</code>, and <code>status</code> views. They are not host Linux files. Directory and hard-link indexes keep ordinary listings O(N); no command scans the host filesystem.</p></section>
  ...
  },
  {
    ...
    html: `
      ...
      <section class="manual-section"><h3>3.5 DOS-style EDIT</h3><p>On the DOS profile, <code>EDIT [path]</code> opens the shared CS QBASIC editor in <code>/EDITOR</code> mode. Bare <code>EDIT</code> starts an <code>UNTITLED</code> buffer and F2 saves it as <code>C:\\NONAME.TXT</code>. Linux does not expose EDIT and uses <code>vi</code>. The top bar exposes File, Edit, Search, Options, and Help; the lower rows show the DOS path, modified state, line, column, and INS/OVR mode. The writer may click to place the cursor, drag to select text, and use the Edit menu or clipboard shortcuts.</p><table><thead><tr><th>Key</th><th>Action</th></tr></thead><tbody><tr><td>F2 / Ctrl+S</td><td>Save and report success or failure</td></tr><tr><td>F10 / Alt+F</td><td>Open keyboard or mouse menus; arrows and Enter select</td></tr><tr><td>Ctrl+F / F3</td><td>Find text / find next with bounded forward search</td></tr><tr><td>Ctrl+C / Ctrl+X / Ctrl+V</td><td>Copy, cut, or paste the bounded current selection</td></tr><tr><td>Ctrl+Home / Ctrl+End</td><td>Move to the start or end of the document</td></tr><tr><td>Ctrl+Left / Ctrl+Right</td><td>Move by word</td></tr><tr><td>Ctrl+Y / Ctrl+Z</td><td>Delete the current line / undo</td></tr><tr><td>Insert</td><td>Toggle insert and overwrite modes</td></tr><tr><td>Alt+F, X</td><td>Exit; a dirty buffer requires Save, Discard, or Cancel</td></tr></tbody></table></section>
...
while True:
  ...
  },
  {
    id: "dos-profile",
    html: `
      <header class="manual-page-header"><p class="manual-kicker">Chapter 14 · Computer System DOS 6.2</p><h2>Operating CS-DOS 6.2 on the CS386SX portable</h2><p class="manual-lead">Computer System DOS 6.2 (CS-DOS 6.2) is the portable profile for a CS386SX 16 MHz machine with 2 MiB RAM. It supports ASM, C, C++, CS QBASIC, and bounded batch files. User MicroPython is unavailable. Storage, identity, execution limits, and the host security boundary remain shared with the CS-Linux desktop.</p></header>
...
quota</code></pre><p><b>Verify:</b> Record processor identity and clock, command status, live memory, and disk use before and after one controlled input sequence.</p><p><b>Expect:</b> Output remains correct, idle event waiting does not create a polling storm, and resource use remains within the independent ceilings.</p></section>
  ...
  },
  {
    ...
    html: `
      ...
      <section class="manual-section"><h3>13.2 Archived pre-CSIR strength-reduction benchmark</h3><p>These are historical live BDS measurements captured through the Computer System MCP on 2026-07-15, before the dedicated C/C++ tokenizer, typed AST, CSIR optimizer, and register allocator shipped. The C and C++ rows are retained as pre-CSIR provenance and must not be used as current compiler measurements; rerun every hardware profile before publishing a current comparison. The BASIC rows likewise record the former cross-profile <code>basic</code> command and do not imply BASIC availability on current CS-Linux or parity with the current CS QBASIC frontend. All sources compute <code>sum(i*i + 3*i + 7)</code> for <code>i = 1..1500</code>. The baseline performs two multiplications per iteration; the optimized source advances the term with first differences. Every supported run started cold and printed the same checksum, <code>1129513000</code>. Compile time, Web transport, browser rendering, Minecraft wall time, and MCP overhead are excluded. Each cell is <b>instructions · modeled CPU cycles · virtual microseconds</b>; the final column is cycle reduction.</p>
