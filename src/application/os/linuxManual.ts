...
const pages = Object.freeze([
  page(
    ...
    [
      ...
      "A foreground pipeline runs each stage as a scheduler-owned guest process joined by a fixed 4 KiB byte ring. | connects stdout, |& connects stdout and stderr, the last stage supplies pipeline status, and a writer whose final reader closes terminates with SIGPIPE status 141. pipefail and background pipelines are unavailable.",
      "Redirects are applied from left to right. Supported forms are <, >, >>, 2>, 2>>, and 2>&1; therefore probe >all 2>&1 combines both streams while probe 2>&1 >out leaves duplicated stderr on the previous stdout. Setup is validated and committed before execution, > truncates at shell-open time, and at most 16 redirect operations are accepted per stage.",
    ],
