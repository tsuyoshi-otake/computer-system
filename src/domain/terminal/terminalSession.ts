...
export class TerminalSession {
  ...
  submitLine(line: string): TerminalLineEvent | undefined {
    if (this.finalization !== undefined) return undefined;
  ...
  finalizeClose(reason: TerminalCloseReason): TerminalFinalization {
    ...
    if (reason === "ClientClosed") return this.finish({ kind: "cancelled" });
    if (reason === "UserBusy") return this.finish({ kind: "competing_form" });
    return this.finish({
  ...
  finalizeFailure(error: unknown, playerValid: boolean): TerminalFinalization {
    if (this.finalization !== undefined) return this.finalization;
    if (!playerValid) return this.finish({ kind: "disconnected" });
    return this.finish({
      kind: "failed",
