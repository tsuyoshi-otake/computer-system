# Issue #12: CS QBASIC 1.0 and DOS editor convergence
...
Computer System ships an original, sandboxed **CS QBASIC 1.0** implementation
for CS-DOS. It does not redistribute Microsoft `QBASIC.EXE`, `QBASIC.HLP`,
fonts, screens, examples, or other binary/assets. Compatibility means source,
command, editing, and debugging behavior within the documented ledger below; it
does not mean native 16-bit x86, DOS interrupts, or byte-for-byte Microsoft
binaries.
...
| Area           | CS QBASIC 1.0 target                                                                                                                                                                             | Deliberate boundary                                                                                                                                |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Legal identity | product name is `CS QBASIC 1.0`; manual states compatibility target and non-affiliation                                                                                                          | no Microsoft binary, help database, branding, or copied screen assets                                                                              |
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Implemented, host-verified    | DOS-only command/image boundary; all researched command-line switches and invalid combinations; Welcome/file/new-buffer/`/RUN` entry; bounded EDIT and separate WorkBench menu states; save, Save As, F1 help, search, undo, dirty exit; writer-owned pointer transport; box-bounded menu hit testing; click/drag selection and bounded clipboard; F7 persistent `.CSX` build; Ctrl+F5 run-last; Shift+F5 build/run; F4 output; in-WorkBench instruction debugger with F5 continue, F8 trace, F9 current-EIP breakpoint, registers, and disassembly; bounded numeric/console compiler; numbered/named jumps, integer expressions and structured loops; explicit dangerous-device rejection |
| Deliberately explicit partial | WorkBench debugging is instruction-level only. It reuses the validated bounded debugger and never claims source-line mapping, source variables, run-to-cursor, or step-over. F1 is bounded original help rather than the Microsoft help database.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Not yet implemented           | Immediate-window execution; procedure navigation; source-level continue/run-to-cursor/step/step-over; string variables/expressions; arrays; `INPUT`; `DATA`/`READ`; floating point; `SUB`/`FUNCTION`; QBasic file handles; graphics/sound; user-program keyboard/mouse API; terminal-cell/framebuffer/window API; scrollbar pointer behavior                                                                                                                                                                                                                                                                                                                                               |
| Verified shipped slice        | Real Chrome delivered one keyboard event and an ordered five-event drag for the writer, then delivered neither after the same session became a viewer. The isolated real-BDS probe passed twice, and `npm run validate` passed 143 test files / 896 tests plus both production builds. These checks do not promote any not-yet-implemented language, source-level debugging, graphics, sound, or program-mouse row.                                                                                                                                                                                                                                                                        |
