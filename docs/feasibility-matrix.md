...
| Connected monitor rendering and touch | supported_with_constraint | Host tests prove topology/touch mapping and bounded updates; BDS twice discovered a 3x2/51x18 surface; GDK touches produced changing cell coordinates and opened DDUI                        | Use a world block frame plus DDUI terminal fallback; coalesce changed cells and cap each flush at 16 |
...
| Persistent item and block identity    | supported_with_constraint | The non-stackable Pocket Computer retained its ID through container, item-entity, placed-block mapping, block-to-item round trip, and full BDS restart                                       | Computer and pocket items must be non-stackable; store block identity by dimension/location          |
...
| Pocket computer lifecycle             | supported                 | Host tests cover use, inventory/container movement, drop, disconnect, duplicates, and bounded O(active) reconciliation; BDS proves custom-item persistence; GDK item use opened the terminal | Use event-driven ownership updates and keep only open sessions in the reconciliation set             |
...
world-facing text or arbitrary texture drawing surface. The tested monitor
fallback discovers a rectangular 3x2 frame, maps its north-face interactions to
a 51x18 cell grid, emits `monitor_touch` coordinates, and opens the same DDUI
terminal surface. Updates are coalesced in a 64-cell queue and flushed at most
16 cells per pass. Distance-dependent visual cost does not apply to DDUI; a
custom world renderer remains out of scope for the stable Add-On baseline.
