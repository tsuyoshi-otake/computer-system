...
Normal release-pack target: Minecraft Bedrock 1.26.30+, `@minecraft/server`
2.8.0, and `@minecraft/server-ui` 2.1.0, without Beta APIs. The managed-BDS
runtime-worker service is a separate operator boundary described below.
...
identifiers (one per digital face mask), preserving the normal release pack's
no-experiments baseline. The managed runtime-worker build separately imports the
Beta server-admin/server-net modules; it never silently enables experiments on a
