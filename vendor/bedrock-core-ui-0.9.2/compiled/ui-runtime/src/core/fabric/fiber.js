...
export function createFiber(id, player) {
    const fiber = {
        id,
        hookStates: [],
        hookIndex: 0,
 ...
 */
export function getFibersForPlayer(player) {
    const fibers = [];
 ...
 */
export function activateFiber(fiber, fn) {
 ...
 */
export function runInFiber(fiber, cb) {
    const [prevFiber, prevDispatcher] = getCurrentFiber();
    setCurrentFiber(fiber, fiber.dispatcher);
    try {
        return cb();
