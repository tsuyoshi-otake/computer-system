...
import { isTextElementType, safeLabelText } from '../../../components/Text';
import { MODAL_DROPDOWN_SLOT_TYPE, MODAL_FORM_BUTTON_SLOT_TYPE, MODAL_INLINE_SELECT_SLOT_TYPE, MODAL_INPUT_SLOT_TYPE, MODAL_SLIDER_SLOT_TYPE, MODAL_TOGGLE_SLOT_TYPE, } from '../../../components/Form';
import { MAX_POOLED_SCROLLS, SCROLL_SLOT_TYPE } from '../../../components/Scroll';
...
function processOverflowText(td, availableWidth) {
    ...
    if (td.maxLines !== undefined) {
        ...
        if (lines.length > td.maxLines) {
            ...
            kept[kept.length - 1] = ellipsizeText(kept[kept.length - 1], availableWidth, td.font, td.scale);
 ...
 */
const MODAL_CONTROL_DEFAULT_HEIGHT = {
...
function layoutScrollContent(slot, axis, viewportWidth, viewportHeight, index) {
    ...
    if (axis === 'x') {
        ...
        syntheticRoot = createNode({ flexDirection: 'row', width: HORIZONTAL_EXTENT_BOUND, height: viewportHeight }, childNodes);
...
export function computeLayout(tree) {
    ...
    if (slots.length > MAX_POOLED_SCROLLS) {
        throw new ScrollLimitError(`Too many <Scroll>s: found ${slots.length}, but a render supports at most ${MAX_POOLED_SCROLLS} `
    ...
    if (concreteRoots.length > 1) {
        ...
        const root = createNode({ flexDirection: 'column', width: CANONICAL_SCREEN.width }, concreteRoots.map(c => buildNode(c)));
