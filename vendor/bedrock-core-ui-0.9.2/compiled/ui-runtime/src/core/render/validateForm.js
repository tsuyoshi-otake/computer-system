import { MODAL_FORM_SLOT_TYPE, MODAL_TOGGLE_SLOT_TYPE, MODAL_SLIDER_SLOT_TYPE, MODAL_DROPDOWN_SLOT_TYPE, MODAL_INLINE_SELECT_SLOT_TYPE, MODAL_INPUT_SLOT_TYPE, MODAL_FORM_BUTTON_SLOT_TYPE, } from '../../components/Form';
import { isElement } from '../guards';
import { ModalFormError } from '../types';
/** ActionForm-only interactive host types — illegal inside a modal `<Form>`. */
const ACTION_ONLY_INTERACTIVE_TYPES = new Set(['button', 'item_renderer']);
/** All modal-only control host types — the restriction set this pass enforces. */
const MODAL_CONTROL_TYPE_SET = new Set([
 ...
 */
export function validateForm(tree) {
    walk(tree, false);
}
function walk(node, insideModal) {
    ...
    if (typeof type === 'string') {
        if (type === MODAL_FORM_SLOT_TYPE) {
            if (insideModal) {
                throw new ModalFormError('A `<Form>` cannot be nested inside another `<Form>`. A screen renders a '
        ...
        if (insideModal && ACTION_ONLY_INTERACTIVE_TYPES.has(type)) {
            throw new ModalFormError(`\`${describe(type)}\` is not allowed inside a \`<Form>\`. A modal form accepts only `
        ...
        if (!insideModal && MODAL_CONTROL_TYPE_SET.has(type)) {
            throw new ModalFormError(`\`${describe(type)}\` is a modal-only control and must be rendered inside a \`<Form>\`. `
