...
const typedFontMetrics = fontMetrics;
 ...
 */
export function ellipsizeText(text, maxWidth, font, fontSize = 1.0) {
 ...
 */
export function wrapText(text, maxWidth, font, fontSize = 1.0) {
    const profile = normalizeFont(font);
    const metrics = BASE_METRICS[profile];
    const scaledMax = maxWidth / fontSize;
    let result = '';
 ...
 */
export function measureText({ text, font, fontSize = 1.0, }) {
    const profile = normalizeFont(font);
    const metrics = BASE_METRICS[profile];
    let lineWidth = 0;
    let maxLineWidth = 0;
